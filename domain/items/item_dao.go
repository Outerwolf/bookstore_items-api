...
func (i *Item) Get() rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError(fmt.Sprintf("error when trying to get id ", i.Id), errors.New("database error"))
	}
	bytes, err := result.Source.MarshalJSON()
	if err != nil {
		return rest_errors.NewInternalServerError(fmt.Sprintf("error when trying to parse database response", i.Id), errors.New("database error"))
	}
...
func (i *Item) Search(query queries.EsQuery) ([]Item, rest_errors.RestErr) {
	...
	for index, hit := range result.Hits.Hits {
		...
		if err := json.Unmarshal(bytes, &item); err != nil {
			return nil, rest_errors.NewInternalServerError("error whent rying to parse response", errors.New("database error"))
		}
		items[index] = item
	}
